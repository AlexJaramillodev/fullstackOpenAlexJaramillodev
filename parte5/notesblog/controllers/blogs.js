...
blogsRouter.get('/', async (request, response) => {
  const blogs = await Blog.find({}).populate('usuario', { username: 1, name: 1, _id: 1  })
  response.json(blogs)
})
...
blogsRouter.put('/:id', async (request, response) => {
  ...
  const populatedBlog = await Blog.findById(updatedBlog._id).populate('usuario', { username: 1, name: 1 })
